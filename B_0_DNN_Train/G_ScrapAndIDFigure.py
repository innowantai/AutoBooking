...
def ImageProcessingPart(fileName,path):
    img = PIL.Image.open(os.path.join(oriPath,fileName)).convert('RGB')
    ...
    for kk ,ff in enumerate(fig5): 
        plt.figure()
        plt.imshow(ff)
       # ii = int((np.random.random()*10000)**2) 
       # plt.savefig(path + '\\{}.jpg'.format(ii+1),dpi=100)
        plt.savefig(path + '\\' +  str(kk) + '.jpg',dpi=100)
    plt.close('all')  
    

def TransFigureToArray(path):
    baseH = 50
    digits = [] 
...
AllResult = []
for ii in range(99):
    # Scraping figure from website named Num-variable number
    Num = GetFigure(FolderName) 
    
    # Creating Scraped figure exclusive folder for saving split number-figure
    oriPath = os.path.join(os.getcwd(),FolderName)
    path = os.path.join(oriPath,str(Num)) 
    if not os.path.exists(path ):
        os.mkdir(path)
    
    # Processing Image and save   
    fileName = Num + '.jpg'
    ImageProcessingPart(fileName,path)
    
    
    # Loading split Number-figure and save to np.array and then translate CNN data form
    
    x_test = TransFigureToArray(path)
    x_test = TransDatafromToCNN(x_test)
    X_test = x_test.reshape(-1, 1,32, 44)
    
    # ID Figure Number
    result = model.predict(X_test)
    res = TransResult(result)
    AllResult.append(res)    
    Files.append(fileName)
    
for kk ,vv in enumerate(Files) :
    res = AllResult[kk]
    fileName = Files[kk]
    reName = fileName.split('.')[0] + '_'
    for ii in res:
        reName += str(ii)
    reName = reName + '.jpg'
    os.rename(os.path.join(oriPath,fileName),os.path.join(oriPath,reName))
    shutil.rmtree(os.path.join(oriPath,fileName.split('.')[0]))
