...
#### For check the correctness of training model When the predicted model has trained.
#### This code preprocessing predicted figure that follow a sequence to split each number figure and classify to exclusive folder for check correctness of trainning model 
...
for fileName in files : 
    ...
    rr = r'[\d]*'
    n1 = re.findall(rr,fileName)
    n1 = np.array(n1)
    resultName = int(n1[n1 != ''][0])
    
    path = os.path.join(os.getcwd(),FolderName,str(resultName) + '_' + str(resultNum))
    if not os.path.exists(path ):
        ...
        for kk ,ff in enumerate(fig5): 
           ...
           # plt.savefig(path + '\\{}.jpg'.format(ii+1),dpi=100)
            plt.savefig(path + '\\' + str(resultNum) + '_' + str(kk) + '.jpg',dpi=100)
...
# for kk,ff in enumerate(fig5):    
#     ax = fig.add_subplot(10,12,kk+1,xticks=[],yticks=[])
#     ax.imshow(ff) 
# =============================================================================
