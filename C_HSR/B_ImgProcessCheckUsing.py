...
def splitFigure2(data):         
    rr = CmpVertical(data) 
    
    #### This number is to decide which will be take off  
    ...
    for ii in range(len(po)-1):
        ...
        if index >= 10:
            ...
            if newImg.shape[1] >= thre:
                rr2 = CmpVertical(newImg)
                ss = round((newImg.shape[1] / 2))
                po2 = []
                va = []
                for tt in range(ss-10,ss+10):
                    po2.append(tt)
                    va.append(rr2[tt]) 
                po2 = np.array(po2)
                va = np.array(va)
                ss2 = po2[np.where(va == min(va))][0]
                fig.append(newImg[:,:ss2])
                fig.append(newImg[:,ss2:])   
            else:            
                fig.append(newImg)
    return fig

def CatchingsplitFigure(fig):
    cat = [] 
    for ff in fig:        
        rr = []
        ff[0,:] = 0
...
### Have problem : 16,44,59,28


folderName = '_nonClassify'
...
#     print('Now process : ', num , '/' , str(len(files)))  
#     
# =============================================================================
        
num = 13
fileName = files[num]
oriimg = cv2.imread(os.path.join(path,fileName)) 
img = ImageCurveProcess(oriimg) 
img2 = takeOffBoundary(img)
              
data = takeOffPoints_2(img2)
data = takeOffPoints(data)
data = takeOffPoints_2(data)
data = takeOffPoints(data)

fig = splitFigure(data)
...
# plt.subplot(212)
# plt.plot( rr2) 
# plt.xlim(0,newImg.shape[1])
# =============================================================================




result = CatchingsplitFigure(fig) 
...
for ff in result:
    ...
    plt.savefig(os.path.join(os.path.join(savePath) , str(num) +  '_' + str(ii) + '.jpg'),dpi=100)
    kk += 1 
